class CoursesEnroll
  ...
  uses_routine CollectCourseInfo,           as: :get_course_info
  ...
  def handle
    ...
    fatal_error(code: :user_is_already_a_course_student) if outputs.user_is_course_student

    outputs.course = run(:get_course_info, with: [:teacher_names],
                                           courses: outputs.period.course).outputs.courses.first
  end
