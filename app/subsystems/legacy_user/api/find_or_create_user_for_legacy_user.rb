class LegacyUser::FindOrCreateUserForLegacyUser
  ...
  def exec(legacy_user)
    ss_map = LegacyUser::User.includes(:entity_user).where{user_id == legacy_user.id}.first
    unless ss_map
      user = run(Entity::CreateUser).outputs.user
      ss_map = LegacyUser::User.create(user_id: legacy_user.id, entity_user_id: user.id)
