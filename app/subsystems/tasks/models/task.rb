...
class Tasks::Models::Task < Tutor::SubSystems::BaseModel
  ...
  def handle_task_step_completion!(completion_time: Time.now)
    self.populate_spaced_practice_exercises!(current_time: completion_time)
  end

  def populate_spaced_practice_exercises!(current_time: Time.now)
    spaced_practice_algorithm.call(event: :task_step_completion, task: self, current_time: current_time)
  end
