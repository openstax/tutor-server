class Content::Routines::TagResource
  ...
  def exec(resource, tags, options = {})
    ...
    tagging_class = options[:tagging_class] || "#{resource.class.name}Tag".constantize
    resource_field = resource.class.name.underscore.split('/').last.to_sym
    ...
    set(taggings: new_tags.collect { |tag|
