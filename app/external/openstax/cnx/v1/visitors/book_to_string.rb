...
class OpenStax::Cnx::V1::Visitors::BookToString
  ...
  def pre_order_visit_fragment_exercise_choice(fragment_exercise_choice:, depth:)
    add_elem(depth, "EXERCISE CHOICE  title = #{fragment_exercise_choice.title}")
  end

  def pre_order_visit_fragment_interactive(fragment_interactive:, depth:)
    add_elem(depth, "INTERACTIVE  title = #{fragment_interactive.title}")
  end

  def pre_order_visit_fragment_video(fragment_video:, depth:)
    add_elem(depth, "VIDEO  title = #{fragment_video.title}")
  end
