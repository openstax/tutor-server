class GetDashboard
  ...
  def load_plans(course, start_at_ntz, end_at_ntz, current_time = Time.current)
    result = run(
      :get_plans, owner: course, start_at_ntz: start_at_ntz, end_at_ntz: end_at_ntz
    ).outputs
    ...
    outputs.plans = result.plans.map do |task_plan|
      ...
      task_plan.attributes.symbolize_keys.except(:is_draft, :is_publishing, :is_published).merge(
        ...
        completed_wrq_step_count: task_plan.completed_wrq_step_count,
        ungraded_wrq_step_count: task_plan.ungraded_wrq_step_count,
