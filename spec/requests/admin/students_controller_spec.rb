...
RSpec.describe Admin::StudentsController, type: :request do
  ...
  context 'GET #index' do
    ...
    it 'returns all the students in a course' do
      get admin_course_students_url(course.id)
  ...
  context '#drop/#restore' do
    ...
    it 'drops a student' do
      expect do
        delete admin_student_url(student.id)
      end.to change { student.reload.deleted? }.from(false).to(true)

      expect(response).to redirect_to edit_admin_course_url(course, anchor: 'roster')
    end

    it 'restores a student' do
      student.destroy
      expect do
        put restore_admin_student_url(student.id)
