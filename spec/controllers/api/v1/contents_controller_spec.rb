...
RSpec.describe Api::V1::ContentsController, type: :controller, api: true,
  ...
  context 'with a fake book' do
    ...
    describe "#ecosystem_readings" do
      it 'raises SecurityTransgression if user is anonymous or not in the course' do
        expect {
          api_get :ecosystem_readings, nil, parameters: { id: ecosystem.id }
        }.to raise_error(SecurityTransgression)

        expect {
          api_get :ecosystem_readings, user_1_token, parameters: { id: ecosystem.id }
        }.to raise_error(SecurityTransgression)
      end
      ...
      it 'works for students in the course' do
        ...
        api_get :ecosystem_readings, user_2_token, parameters: { id: ecosystem.id }
        expect(response).to have_http_status(:success)
        student_response = response.body_as_hash
  ...
  context 'with a real book' do
    ...
    describe "#ecosystem_exercises" do
      it 'raises SecurityTransgression if user is anonymous or not a teacher' do
        page_ids = Content::Models::Page.all.map(&:id)

        expect {
          api_get :ecosystem_exercises, nil, parameters: { id: @ecosystem.id, page_ids: page_ids }
        }.to raise_error(SecurityTransgression)

        expect {
          api_get :ecosystem_exercises, user_2_token, parameters: { id: @ecosystem.id,
                                                                    page_ids: page_ids }
        }.to raise_error(SecurityTransgression)
      end

      it "should return an empty result if no page_ids specified" do
        api_get :ecosystem_exercises, user_1_token, parameters: { id: @ecosystem.id }
