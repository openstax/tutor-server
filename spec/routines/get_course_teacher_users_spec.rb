require 'rails_helper'

describe GetCourseTeacherUsers, type: :routine do

  context "when a course has no teachers" do
    let(:target_course) { CreateCourse.call(name: 'target').course }
    let(:other_course)  { CreateCourse.call(name: 'other').course }
    let(:other_user)    { FactoryGirl.create(:user) }

    before(:each) do
      result = AddUserAsCourseTeacher.call(course: other_course, user: other_user)
      expect(result.errors).to be_empty
    end

    it "should return an empty array" do
      result = GetCourseTeacherUsers.call(target_course)
      expect(result.errors).to be_empty
      expect(result.teachers).to be_empty
    end
  end

  context "when a course has one teacher" do
    let(:target_course) { CreateCourse.call(name: 'target 2').course }
    let(:other_course)  { CreateCourse.call(name: 'other 2').course }
    let(:target_user)   { FactoryGirl.create(:user) }
    let(:other_user)    { FactoryGirl.create(:user) }

    before(:each) do
      result = AddUserAsCourseTeacher.call(course: other_course, user: other_user)
      expect(result.errors).to be_empty
      result = AddUserAsCourseTeacher.call(course: target_course, user: target_user)
      expect(result.errors).to be_empty
    end

    it "should return an array containing that teacher" do
      result = GetCourseTeacherUsers.call(target_course)
      expect(result.errors).to be_empty
      expect(result.teachers.size).to eq(1)
      expect(result.teachers).to include(target_user)
    end
  end

  context "when a course has multiple teachers" do
    let(:target_course) { CreateCourse.call(name: 'target 3').course }
    let(:other_course)  { CreateCourse.call(name: 'other 3').course }
    let(:target_user1)  { FactoryGirl.create(:user) }
    let(:target_user2)  { FactoryGirl.create(:user) }
    let(:other_user)    { FactoryGirl.create(:user) }

    before(:each) do
      result = AddUserAsCourseTeacher.call(course: other_course, user: other_user)
      expect(result.errors).to be_empty
      result = AddUserAsCourseTeacher.call(course: target_course, user: target_user1)
      expect(result.errors).to be_empty
      result = AddUserAsCourseTeacher.call(course: target_course, user: target_user2)
      expect(result.errors).to be_empty
    end
    it "should return an array containing those teachers" do
      result = GetCourseTeacherUsers.call(target_course)
      expect(result.errors).to be_empty
      expect(result.teachers.size).to eq(2)
      expect(result.teachers).to include(target_user1)
      expect(result.teachers).to include(target_user2)
    end
  end

end
