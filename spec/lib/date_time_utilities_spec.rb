...
RSpec.describe DateTimeUtilities, type: :lib do
  let(:central_time) { ActiveSupport::TimeZone['US/Central'] }
  let(:pacific_time) { ActiveSupport::TimeZone['US/Pacific'] }
  ...
  context 'when reading strings with timezones' do
    it 'infers UTC if the string has no timezone info' do
    ...
    it 'reads timezone from offset' do
    ...
    it 'reads timezone from name' do
    ...
    it 'reads timezone of the form -05' do
      date_time = described_class.from_s("2016-12-25 7:00 -05")
      expect(date_time.zone).to eq "-05:00"
      expect(date_time.to_s).to eq "2016-12-25T07:00:00-05:00"
    end

    it 'reads timezone of the form -05:00' do
  ...
  context 'when removing timezone information' do
    it 'drops timezones including their offsets' do
      date_time = described_class.from_s("2016-12-25 7:00 -0500")
  ...
  context 'when applying timezone information' do
    it 'applies timezones to UTC date_times' do
  ...
  context '#parse_in_zone' do
    it 'parses a date correctly' do
      date_time = described_class.parse_in_zone(string: '5/25/17', zone: 'US/Eastern')
