...
RSpec.feature 'DistrictTermsWork' do
  ...
  scenario 'blah' do
    ...
    expect(FinePrint).to receive(:unsigned_contracts_for)
                           .with(user_1, name: ['general_terms_of_use'])
    ...
    expect(FinePrint).to receive(:unsigned_contracts_for)
                           .with(user_2, name: ['general_terms_of_use', 'privacy_policy'])
