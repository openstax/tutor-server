...
RSpec.describe 'PushSalesforceCourseStats', vcr: VCR_OPTS do
  before(:all) do
    ...
    @chemistry_offering = FactoryBot.create(:catalog_offering, salesforce_book_name: 'Chemistry')

    @course = FactoryBot.create(
      :course_profile_course,
      name: 'A Fun Course',
      term: :spring,
      year: 2017,
    ...
    @period_uuids = [ @period1_uuid, @period2_uuid, @period3_uuid ]
  ...
  context 'when some teacher has a Salesforce Contact' do
    ...
    before do
      @user.account.update_attribute :salesforce_contact_id, sf_contact_a.id
    ...
    context 'when there is an existing TutorCoursePeriod' do
      ...
      it 'is ok with another course being in the same term same teacher' do
        ...
        AddUserAsCourseTeacher[course: other_course, user: @user]
