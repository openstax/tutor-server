...
RSpec.describe Admin::CoursesRemoveSalesforce, type: :handler do
  ...
  it 'removes and resets stats' do
    ...
    klass = OpenStax::Salesforce::Remote::OsAncillary
    fake_sf_object_1 = fake_sf_object(klass: klass, id: 'earlier')
    fake_sf_object_1.num_students = 42
    fake_sf_object_2 = fake_sf_object(klass: klass, id: 'later')
    fake_sf_object_2.num_students = 38

    [course, period_1].each do |tutor_object|
      FactoryBot.create(
        :salesforce_attached_record, tutor_object: tutor_object, salesforce_object: fake_sf_object_1
      )
    end

    [course, period_2].each do |tutor_object|
      FactoryBot.create(
        :salesforce_attached_record, tutor_object: tutor_object, salesforce_object: fake_sf_object_2
      )
    end

    expect do
      call(course_id: course.id, salesforce_id: 'earlier')
    end.to change { Salesforce::Models::AttachedRecord.without_deleted.count }.by(-2)

    expect(Salesforce::Models::AttachedRecord.without_deleted.map(&:tutor_gid)).to contain_exactly(
      course.to_global_id.to_s, period_2.to_global_id.to_s
    )

    # Course AR should be soft deleted, period AR should be really deleted
    expect(Salesforce::Models::AttachedRecord.all.map(&:tutor_gid)).to contain_exactly(
      course.to_global_id.to_s, course.to_global_id.to_s, period_2.to_global_id.to_s
    )
  ...
  it 'does not explode if the SF object is no longer in SF' do
    ...
    FactoryBot.create(:salesforce_attached_record,
                       tutor_object: course,
                       salesforce_class_name: "OpenStax::Salesforce::Remote::OsAncillary",
                       salesforce_id: "something",
                       salesforce_object: nil)

    allow(OpenStax::Salesforce::Remote::OsAncillary).to receive(:find).with("something") { nil }

    expect { call(course_id: course.id, salesforce_id: 'something') }.not_to raise_error
  end
  ...
  it "can find the methods it needs in potential SF object classes" do
    ...
    expect(OpenStax::Salesforce::Remote::OsAncillary.new).to respond_to(:reset_stats, :save)
  end
