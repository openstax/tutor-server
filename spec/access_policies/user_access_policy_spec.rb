...
RSpec.describe UserAccessPolicy, :type => :access_policy do
  ...
  context 'index' do
    it 'cannot be accessed by anonymous users' do
      expect(UserAccessPolicy.action_allowed?(:index, anon, UserProfile::Profile)).to eq false
    end

    it 'can be accessed by applications and human users' do
      expect(UserAccessPolicy.action_allowed?(:index, user, UserProfile::Profile)).to eq true

      expect(UserAccessPolicy.action_allowed?(:index, application, UserProfile::Profile)).to eq true
    end
  end

  context 'show, update and destroy' do
    it 'cannot be accessed by anonymous users or applications' do
      expect(UserAccessPolicy.action_allowed?(:read, anon, UserProfile::Profile)).to eq false
      expect(UserAccessPolicy.action_allowed?(:update, anon, UserProfile::Profile)).to eq false
      expect(UserAccessPolicy.action_allowed?(:destroy, anon, UserProfile::Profile)).to eq false
