...
RSpec.describe Content::Api::SearchLocalExercises, :type => :routine,
                                                   :vcr => VCR_OPTS do
  ...
  it 'can search imported exercises' do
    Content::ImportPage.call(cnx_page: cnx_page,
                                       book_part: book_part)

    url = Content::Exercise.first.url
    exercises = Content::Api::SearchLocalExercises.call(url: url).outputs
                                                                 .items
    expect(exercises.length).to eq 1
    expect(exercises.first.url).to eq url

    lo = 'k12phys-ch04-s01-lo01'
    exercises = Content::Api::SearchLocalExercises.call(tag: lo).outputs
                                                                .items
    ...
    exercises = Content::Api::SearchLocalExercises.call(tag: embed_tag).outputs
                                                                       .items
    expect(exercises.length).to eq 1
