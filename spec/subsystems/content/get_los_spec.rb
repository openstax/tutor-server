...
RSpec.describe Content::GetLos, :type => :routine do
  ...
  let!(:lo_1)   { FactoryGirl.create :content_tag, value: 'lo01', tag_type: :lo }
  let!(:lo_2)   { FactoryGirl.create :content_tag, value: 'lo02', tag_type: :lo }
  let!(:lo_3)   { FactoryGirl.create :content_tag, value: 'lo03', tag_type: :lo }
  let!(:lo_4)   { FactoryGirl.create :content_tag, value: 'lo04', tag_type: :lo }
  let!(:lo_5)   { FactoryGirl.create :content_tag, value: 'lo05', tag_type: :lo }
  ...
  it "should get the LO's for a single page" do
    ...
    expect(result.outputs.los).to include(lo_1.value)
    expect(result.outputs.los).to include(lo_2.value)
    expect(result.outputs.los).not_to include(lo_3.value)
    expect(result.outputs.los).not_to include(lo_4.value)
    expect(result.outputs.los).not_to include(lo_5.value)

    result = Content::GetLos.call(page_ids: page_2.id)
    expect(result.errors).to be_empty
    expect(result.outputs.los).not_to include(lo_1.value)
    expect(result.outputs.los).not_to include(lo_2.value)
    expect(result.outputs.los).to include(lo_3.value)
    expect(result.outputs.los).not_to include(lo_4.value)
    expect(result.outputs.los).not_to include(lo_5.value)
  end
  ...
  it "should get the LO's for a single book_part" do
    ...
    expect(result.outputs.los).not_to include(lo_2.value)
    expect(result.outputs.los).not_to include(lo_3.value)
    expect(result.outputs.los).to include(lo_4.value)
    expect(result.outputs.los).to include(lo_5.value)
  end
