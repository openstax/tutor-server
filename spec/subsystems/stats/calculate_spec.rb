...
RSpec.describe Stats::Calculate, type: :routine do
  ...
  let!(:reading_step) do
    FactoryBot.create :tasks_task_step, first_completed_at: 1.day.ago,
                                        tasked_type: :tasks_tasked_reading
  end

  let!(:homework_step) do
    FactoryBot.create :tasks_task_step, first_completed_at: 1.day.ago,
  ...
  before do
    ...
    add_students[task_plan.course]
    add_students[homework_step.task.task_plan.course]
    add_students[reading_step.task.task_plan.course]
  end
  ...
  it 'counts all the things' do
    ...
    expect(interval.courses.active).to include task_plan.course
