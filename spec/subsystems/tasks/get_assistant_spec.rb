...
RSpec.describe Tasks::GetAssistant, type: :routine do
  ...
  let!(:assistant)        { FactoryBot.create :tasks_assistant }
  let!(:course_assistant) {
    FactoryBot.create :tasks_course_assistant, course: course,
                                                assistant: assistant,
                                                tasks_task_plan_type: 'dummy'
  }
  let(:task_plan)        { FactoryBot.build :tasks_task_plan, type: 'dummy' }
  let(:hw_task_plan)     { FactoryBot.build :tasks_task_plan, type: 'homework' }
