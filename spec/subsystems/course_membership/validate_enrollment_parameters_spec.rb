...
describe CourseMembership::ValidateEnrollmentParameters, type: :routine do
  ...
  let!(:user)      { profile = FactoryGirl.create :user_profile
  ...
  it "returns false if book_uuid is invalid even if enrollment_code is" do
    expect( described_class[book_uuid: 'hackme', enrollment_code: period.enrollment_code] ).to eq(false)
  end

  it "returns true if both book_uuid and enrollment_code is valid" do
    expect( described_class[book_uuid: book.uuid, enrollment_code: period.enrollment_code] ).to eq(true)
  end
